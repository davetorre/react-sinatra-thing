...
var insertHtml = function insertHtml(elementId, html) {
  var element = document.getElementById(elementId);
  element.innerHTML = '';
  element.insertAdjacentHTML('afterbegin', html);
};

var noteItem = function noteItem(note) {
  return '<li class="note-item">' + note.title + '</li>';
};

var noteList = function noteList(notes) {
  var items = notes.reduce(function (acc, note) {
    return acc + noteItem(note);
  }, '');
  return '\n    <h1>Notes</h1>\n    <ul>' + items + '</ul>\n  ';
};

var noteDetail = function noteDetail(note) {
...
[].forEach.call(noteItemEls, function (el, index) {
